...
export default async function handler(req, res) {
    ...
    if (type === 'Orçamento') {
        subject = isPt ? 'O teu orçamento — LUMI Atelier' : 'Your quote — LUMI Atelier';
        html = buildBudgetEmail({ isPt, name, budget, session_url });
    } else if (type === 'Mais Detalhes') {
        subject = isPt ? 'Precisamos de mais detalhes — LUMI Atelier' : 'A few more details needed — LUMI Atelier';
        html = buildDetailsEmail({ isPt, name, message });
    } else if (type === 'Recusado') {
        subject = isPt ? 'O teu pedido — LUMI Atelier' : 'Your request — LUMI Atelier';
        ...
        <tr>
          <td style="padding:0 48px 32px;">
            <p style="margin:0;font-size:10px;color:#B09080;line-height:1.6;">
              ${isPt ? 'Recebeste este email porque preencheste o formulário de marcação em lumiatelierpt.com.' : 'You received this email because you submitted the booking form at lumiatelierpt.com.'}
            </p>
          </td>
        </tr>
...
function buildBudgetEmail({ isPt, name, budget, session_url }) {
    const para1 = isPt
        ? 'Analisámos a tua ideia com toda a atenção e ficamos felizes em poder avançar com este projeto.'
        : "We've carefully reviewed your idea and we're glad to move forward with this piece.";

    const para2 = isPt
        ? 'Para confirmar a tua sessão, pedimos um depósito de <strong>20€</strong> via MB Way para o número <strong>932 558 951</strong>. O depósito é não reembolsável e a sessão só fica confirmada após a sua receção.'
...
function buildDetailsEmail({ isPt, name, message }) {
    const para1 = isPt
        ? 'Obrigada pelo teu pedido. Analisámos a tua ideia, mas precisaríamos de alguns detalhes adicionais antes de poder avançar.'
        : "Thank you for your request. We've reviewed your idea carefully, but we'd need a few more details before moving forward.";

    const para3 = isPt
        ? 'Responde a este email com as informações pedidas e voltamos a entrar em contacto em breve.'
        : "Reply to this email with the requested information and we'll be back in touch soon.";
...
function buildRejectionEmail({ isPt, name, reason }) {
    const intro = isPt
        ? 'Obrigada por teres pensado em nós para este projeto. Analisámos o teu pedido com atenção.'
        : "Thank you for considering us for this piece. We've looked carefully at your request.";

    const closing = isPt
        ? 'Desejamos-te boa sorte na tua procura pelo artista certo.'
        : 'We wish you all the best in finding the right artist for this piece.';

    let reasonText;
    if (reason === 'Fora do nosso estilo') {
        reasonText = isPt
            ? 'Infelizmente, este projeto não se enquadra no nosso estilo artístico atual. Para te garantir o melhor resultado possível, preferimos não avançar com este pedido.'
            : "Unfortunately, this project isn't a fit for our current artistic style. To ensure you get the best possible result, we prefer not to move forward.";
    } else if (reason === 'Não conseguimos fazer cover') {
        reasonText = isPt
            ? 'Após analisar a tatuagem existente, não nos é possível garantir um resultado satisfatório numa cover neste caso específico. Preferimos ser honestos do que arriscar um resultado que não te faria justiça.'
            : "After reviewing the existing tattoo, we're not confident we can guarantee a satisfying result for a cover-up in this particular case. We prefer honesty over a result that wouldn't do you justice.";
    } else {
        reasonText = isPt
            ? 'Neste momento a nossa agenda não tem disponibilidade para novas marcações com a urgência que procuras. Podes ficar atento às nossas redes sociais para futuras aberturas de agenda.'
            : "At this time, our schedule doesn't have availability for new bookings with the urgency you're looking for. Keep an eye on our social media for future schedule openings.";
    }
...
function buildDepositEmail({ isPt, name }) {
    const para1 = isPt
        ? 'Recebemos o teu depósito de 20€. A tua sessão está confirmada.'
        : "We've received your 20€ deposit. Your session is now confirmed.";

    const para2 = isPt
        ? 'Ficamos muito felizes por trabalhar contigo em breve. Qualquer dúvida, estamos à tua disposição.'
        : "We're truly looking forward to working with you. Feel free to reach out if you have any questions.";
